...
function getFileNamesInline(text) { // changed from 'elem'
  ...
  for (var p in paths) {
      var path = paths[p];
      var fixed = path.match(/^"(?:\\\\\?|)[^"]+[\\\/]([^\\\/"]+)"$/gim); //.match(/^"?[A-Z]:\\[^]+?[\\\/]([^\\\/"]+)"?$/gi);
  if (!fixed) {continue}
      fixed = path.replace(/^"(?:\\\\\?|)[^"]+[\\\/]([^\\\/"]+)"$/gim,"$1"); //.replace(/^"?[A-Z]:\\[^]+?[\\\/]([^\\\/"]+)"?$/gi,"$1");
...
function setListeners() {
  ...
  document.addEventListener("paste", async (e) => {
      if (e.target.tagName === "TEXTAREA" && e.target.id.match(/div\d+_input\d+/i)) {
        navigator.clipboard.readText().then((text) => {
          var res = getFileNamesInline(text);
          console.log(res)
          if (text === res) { return } // don't replace text if no change
          e.target.value = res;
        })
      }
  });
  ...
  for (var t in copyFields) {
    if (copyFields[t].id) {
      copyFields[t].addEventListener("keyup", (e) => {
        var curr = e.target;
        copies[copies.currentSet][curr.id] = curr.innerHTML;
      });
    }
  }
}
