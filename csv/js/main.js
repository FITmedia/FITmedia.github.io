var db = {
    ...
    locates: {
      ...
      load: () => { 
          let record = mergeRecChanges();
          let state = record["STATE_CD"];
          let dbSt = db.locates.byState;
          let func = dbSt[state] || dbSt["ANY"];
          let items = [func(record)];
          setCopyItems(items, true);
      },
      byState: {
          "ANY": (record) => {
            var end = (record["LATITUDE"] !== "" && record["LONGITUDE"] !== "") ? " <br><br>"+record["LATITUDE"]+", "+record["LONGITUDE"] : "";
            var rear = "";
            var dog = "";
            if (record["LOT_NBR"].match(/(REAR|FRONT)/gi)) {
                rear = `, ${record["LOT_NBR"].match(/(REAR|FRONT)/gi)[0]} EASEMENT`;
            }
            //var rear = record["LOT_NBR"].match(/(REAR|FRONT)/gi) ? `, ${record["LOT_NBR"].match(/REAR/gi)[0]} EASEMENT` : ", FRONT EASEMENT";
            if (record["REMARKS"].match(/\bDOGS*\b/gi)) {
              dog = `, HAS ${record["REMARKS"].match(/\bDOGS*\b/gi)[0]}`;
            }
            var where = record["REMARKS"]
              .replace(/MOVED TO .+ ON [0-9]+\-[a-z]+\-[0-9]+ */gi,"")
              .match(/(FROM\s.+?TO\s.+?|FROM\s.+?|TO\s.+?)((?=[,\.\/])|$)/gi);
            where = (where) ? " "+where[0] : " AS LAID"; //`[${record["REMARKS"]}]`;
            if (where.match(/[\[\]]/g)) {
                var text = where.match(/\[.+?\]/g)[0].replace(/[\[\]]/g,"");
                //end = `<br><input onkeyup="replacer(this,'${text}')" placeholder="${text}">`;
            }
            var txt = `MARK FOR BURYING PHONE LINE${where}${rear}, TERM AT ${record["TERMINAL_ADDR"]}, APPROX FOOTAGE IS ${record["APPROX_WIRE_FT"]} FT${dog}`;
            return `<div>${txt}</div>`+end; 
          },
          "IN": (record) => {
            var end = (record["LATITUDE"] !== "" && record["LONGITUDE"] !== "") ? " <br><br>"+record["LATITUDE"].replace(/(-*\d+\.\d{5})\d*/g,"$1")+", "+record["LONGITUDE"].replace(/(-*\d+\.\d{5})\d*/g,"$1") : "";
            var rear = "";
            var bore = "";
            var dog = "";
            var dogMatch = record["REMARKS"].match(/\bDOGS*\b/gi);
            if (record["LOT_NBR"].match(/(REAR|FRONT)/gi)) {
                rear = `, ${record["LOT_NBR"].match(/(REAR|FRONT)/gi)[0]} EASEMENT`;
            }
            if (record["TERMINAL_ADDR"].match(/\d{1,5}( |-)*\d* ([NESW]{1,2}) [a-zA-Z]{3,}/g)
                && record["STREET_ADDR"].match(/\d{1,5} (?![NESW]{1,2} )[a-zA-Z]{3,}/g)) {
                var stDir = record["TERMINAL_ADDR"].match(/\d{1,5} *\d* ([NESW]{1,2} )/)[1];
                let st = record["STREET_ADDR"].match(/(\d{1,5} )(.*)$/);
                let text = st[1] + stDir + st[2];
                let span = document.createElement("span");
                span.innerHTML = text;
                span.style = "position: absolute; margin: 20px 30px; z-index: 300; color: red; font-size: 0.75rem;";
                document.querySelector("#STREET_ADDR").before(span);
            }
            record["TERMINAL_ADDR"] = record["TERMINAL_ADDR"].replace(/\bAV\b/,"AVE");
            if (record["BORE_REQ_IND"] === "T") {
              bore = `, BORE UNDER ${record["PUSH_BORED_NM"]} FOR APPROX ${record["BORE_LENGTH"]} FEET`;
            }
            var where = record["REMARKS"]
              .replace(/^(?:BURY|PLEASE BURY) ?(?:DROP|) ?(.+?)(?: TY| *$)/g,"$1")
              .replace(/(SLACK |)(NI\b|NID)/g,"HOUSE BOX")
              .replace(/TERM\b/g,"TERMINAL")
              .replace(/PED\b/g, "PEDESTAL")
              .replace(/LEAVE ANY SLACK( AT HOUSE|)( TO BE REMOVED|)/gi,"TO HOUSE BOX")
              .replace(/EXISTING [A-Z ]*(WIRE|LINE|ONLY)/g,"AS LAID")
              .replace(/(?:PROPERLY |)TO ATT STANDARDS (ALONG PROPERTY LINE |)(?:AND TO PROPER DEPTH |)(?:AND LEAVE SLACK AT |)/gi,"$1 TO ")
              .replace(/TO SPLICE SHORTER( GET LOCATES FOR OTHER DROPS IN AREA|)( *DO| *DR|)/g,"TO HOUSE BOX")
              .replace(/MOVED TO QUALSAT ON [0-9]+\-[a-z]+\-[0-9]+ */gi,"");
            var matches = where.match(/(FROM\s.+?TO\s.+?|FROM\s.+?|TO\s.+?)((?=[,\.\/])|$)/gi);
            if (dogMatch) {
              dog = `, HAS ${dogMatch[0].trim()}`;
              where = where.replace(dogMatch[0],"");
            }
            where = (matches) ? matches[0].trim() : where.trim();
            if (where.match(/[\[\]]/g)) {
                var text = where.match(/\[.+?\]/g)[0].replace(/[\[\]]/g,"");
            }
            var txt = `LOCATE FOR BURYING PHONE LINE ${where}${rear}, TERMINAL AT ${record["TERMINAL_ADDR"]}, APPROX FOOTAGE IS ${record["APPROX_WIRE_FT"]} FEET${bore}${dog}`;
            return `<div>${txt}</div>`+end;
          },
      }
    },
    ...
    if ('files' in x) {
      if (x.files.length == 0) {
        ...
        for (var i = 0; i < x.files.length; i++) {
          ...
          var obj = parseCSV(text,true);
          if (!db[fileName]) {
              db[fileName] = {idHeader: obj.headers[0], ids: [], order: obj.headers};
          }
          db.records = obj;
          db[fileName].ids = Object.keys(db.records);
        }
      }
    } 
    else {
  ...
  function searchRecId(e) {
   ...
   // console.log("Showing '"+fileName+"' headers.");
    ...
    for (var h in show) {
        ...
        /*if (hdr === "LOT_NBR" && !info.innerHTML.match(/rear/i)) {
            info.classList.add("highlight");
        }
        if (hdr === "STATE_CD" && !info.innerHTML.match(/KS/i)) {
            info.classList.add("highlight");
        }*/
   ...
   /* submit.addEventListener("click", (e) => {
        ...
        var id = document.getElementById(idHeader).value;
        for (var f in fields) {
            var field = fields[f];
